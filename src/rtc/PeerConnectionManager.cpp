...
PeerConnectionManager::PeerConnectionManager(TcpSignalingClient* signaling, bool isCaller, QObject* parent)
    : QObject(parent)
    , m_signaling(signaling)
    , m_isCaller(isCaller)
{
    connect(m_signaling, &TcpSignalingClient::jsonReceived,
...
void PeerConnectionManager::start()
{
    ...
    if (m_isCaller) {
        ...
        m_pc->setLocalDescription();    // ���� onLocalDescription -> �� offer
...
void PeerConnectionManager::onSignalingMessage(const QJsonObject& obj)
{
    ...
    if (type == "offer" || type == "answer") {
        ...
        if (!m_isCaller && type == "offer") {
            m_pc->setLocalDescription();  // ���� onLocalDescription -> �� answer
        }
