...
signals:
	...
	void frameReady(const uchar* y, const uchar* u, const uchar* v,
...
public slots:
	void processEncodedData(const QByteArray& naluData);
	void onStreamTimeout();//监测流超时
...
private:
	...
	std::string m_myId;
	std::string m_peerId;
	QTimer* m_watchingTimer = nullptr;//看门狗定时变量
	...
	// 【新增】添加三个成员变量作为缓冲区
	// 使用成员变量而不是局部变量，是为了确保在 emit 信号时内存不被销毁
