      ...
      ui(new Ui::shared_screen)
{
    ...
    //测试渲染使用的代码
    m_glScreenWidget = new VideoOpenGLWidget(this);
    //---默认隐藏OpenGL窗口，显示原本提示的Label
    m_glScreenWidget->hide();

    m_glScreenWidget->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    m_glScreenWidget->setMinimumSize(640,480);
    //布局操作，获取ui->screenPreview父布局并把OpenGL窗口加入布局
    QWidget* previewContainer = ui->screenPreview->parentWidget();
    if (previewContainer && previewContainer->layout()) {
        //将OpenGL窗口加入布局
        previewContainer->layout()->addWidget(m_glScreenWidget);
        qDebug() << "OpenGL widget added to layout";
        qDebug() << "Container size:" << previewContainer->size();

    }
    else {
        m_glScreenWidget->setGeometry(ui->screenPreview->geometry());
    }

    //初始化模拟定时器
    m_simulationTimer = new QTimer(this);
    m_simulationTimer->setInterval(33);
    connect(m_simulationTimer,&QTimer::timeout,this,&shared_screen::onSimulateVideoFrame);

    //初始化图案切换定时器
    m_patternSwitchTimer = new QTimer(this);
    m_patternSwitchTimer->setInterval(3000);
    connect(m_patternSwitchTimer, &QTimer::timeout, this, [this]() {
        m_currentPatternType=(m_currentPatternType+1)%4;
        QString patternName;
        switch (m_currentPatternType) {
        ...
        qDebug() << "=====Switching to:" << patternName <<"====";
     });
}
...
shared_screen::~shared_screen()
{
    ...
    if (m_patternSwitchTimer->isActive()) {
        m_patternSwitchTimer->stop();
    }
    delete ui;
}
...
void shared_screen::on_btnVoiceClicked()
{
    ...
    if (isVoiceOn)
    {
        btnVoice->setIcon(QIcon("../icons/录音_voice.png"));
...
void shared_screen::on_btnShareScreenClicked()
{
    ...
    if (isScreenSharing) {
        //开启共享模式
        ui->screenPreview->hide();
        //显示OpenGL渲染窗口
        if (m_glScreenWidget) {
            m_glScreenWidget->setGeometry(ui->screenPreview->geometry());
            m_glScreenWidget->show();
            m_glScreenWidget->raise();
            m_glScreenWidget->update();

            QTimer::singleShot(500, this, [this]() {
                if (m_glScreenWidget->size().height() > 0) {
                    m_simulationTimer->start();
                    qDebug() << " Simulation timer start";
                    
                }
                if (m_patternSwitchTimer) {
                    m_currentPatternType = 0;
                    m_patternSwitchTimer->start();
                    qDebug()<<"Pattern switch timer start";
                }
             });
        }
        ui->statusLabel->setText(u8"正在共享");
    }
    else {
        //关闭共享模式
...
void shared_screen::onSimulateVideoFrame() {
    if (m_glScreenWidget && m_glScreenWidget->isVisible())
        m_glScreenWidget->renderTestPattern(m_currentPatternType);
}